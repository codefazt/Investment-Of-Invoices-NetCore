...
{
    public class ConsultasPagosFacturasVencidasModel : PageModel
    {
        #region Data
        ...
        #endregion
        public ConsultasPagosFacturasVencidasModel(SignInManager<User> signInManager, IGlobalService globalService, IPeopleService peopleService, IPaymentService paymentService, IAuthService aS)
        {
        ...
        public async Task<JsonResult> OnPostPayedInvoices([FromBody]RequestPagination pag)
        {
            ...
            var paymentsQuery = await _paymentService.ReceiptsQueryConfirmant(new ParamConsultaPeyments() { Entity = Confirmant, State = "paid,unpaid", Abbreviation = "OVERDUE", Country = Country, StateInvoice = "finalize" }, pag.Filter, pag.Pagination, token);
            //var unpaymentsQuery = await _paymentService.ReceiptsQueryConfirmant(new ParamConsultaPeyments() { Entity = Confirmant, State = "unpaid", Abbreviation = "OVERDUE", Country = Country, StateInvoice = "finalize" }, pag.Filter, pag.Pagination, token);
