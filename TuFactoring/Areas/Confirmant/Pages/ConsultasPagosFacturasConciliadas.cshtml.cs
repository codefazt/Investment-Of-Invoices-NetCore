...
{
    public class ConsultasPagosFacturasConciliadasModel : PageModel
    {
        #region Data
        ...
        #endregion
        public ConsultasPagosFacturasConciliadasModel(SignInManager<User> signInManager, IGlobalService globalService, IPeopleService peopleService, IPaymentService paymentService, IAuthService aS)
        {
        ...
        public async Task<JsonResult> OnPostPayedInvoices([FromBody]RequestPagination pag)
        {
            ...
            var paymentsQuery = await _paymentService.ReceiptsQueryConfirmant(new ParamConsultaPeyments() { Entity = Confirmant, State = "paid", Abbreviation = "RECONCILED", Country = Country }, pag.Filter, pag.Pagination, token);           
        ...
        public async Task<JsonResult> OnPostUnpayedInvoices([FromBody]RequestPagination pag)
        {
            ...
            var unpaymentsQuery = await _paymentService.ReceiptsQueryConfirmant(new ParamConsultaPeyments() { Entity = Confirmant, State = "unpaid", Abbreviation = "RECONCILED", Country = Country, Method = "TRANSFER", StateInvoiceNot = "overdue" }, pag.Filter, pag.Pagination, token);
