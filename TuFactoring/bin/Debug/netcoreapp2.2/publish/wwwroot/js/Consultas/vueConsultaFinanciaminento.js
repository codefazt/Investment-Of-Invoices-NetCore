var app = new Vue({
    ...
    data: {
        ...
        headerConsulta: [
            ...
            { text: i18n.t("headers.valorTotalLimite"), value: 'available', align: 'center', sortable: true },
            { text: i18n.t("headers.montoSobrante"), value: 'usage', align: 'center', sortable: true },
    ...
    methods: {
                ...
                if (elemento != null) {
                    if (elemento.list != null) {
                        ...
                        if (elemento.list[0] != null) {
                            if (elemento.list[0].lastName != null && elemento.list[0].lastName != "") {
                                this.loading = false
                                this.buscarClientes = false
                                toastr.error(i18n.t("mensajesModal.estimadoUsuario") + "<br /><br />" + elemento.list[0].lastName);
                            }
                        }
                                            ...
                                            for (let j = 0; j < listaEstados.currencies.length; j++) {
                                                if (subElemnt.quotas[i].currency == listaEstados.currencies[j].id) {
                                                    ...
                                                    if (subElemnt.quotas[i].usage < 1) {
                                                        ...
                                                        subElemnt.quotas[i].available > 0 ? usage = subElemnt.quotas[i].available : usage = "0,00";

                                                    } else {
                                                        (subElemnt.quotas[i].available - subElemnt.quotas[i].usage) > 0 ? usage = (subElemnt.quotas[i].available - subElemnt.quotas[i].usage) : usage = "0,00";
                                                    }
        ...
        async detalles_cliente(cliente) {
            ...
            console.log("Activacion por dale al clik")
