new Vue({
    ...
    methods: {
        ...
        async llenarConfirmants() {
            await axios.post('?handler=confirmants', {},
                ...
                .then((respond) => {
                    ...
                    this.Bancos.map(banks => {
                        this.contadorFintech++
                        if (banks.is_fintech && this.contadorFintech == this.Bancos.length) {
                            this.isFintech = true
                        }
                    })
        ...
        //
        async detailQuota() {
            await axios.post('?handler=DetailtQuota', {},
                {
                    headers: {
                        "RequestVerificationToken": $('input:hidden[name="__RequestVerificationToken"]').val()
                    }
                ...
                .then((respond) => {
                    ...
                    this.dataQuotas = respond.data.quotas;
        ...
        async llenarCatalogo() {
            await axios.post('?handler=catalogo', {},
                ...
                .then((respond) => {
                    ...
                    if (this.dataQuotas != null) {
                        for (let w = 0; w < this.dataQuotas.length; w++) {
                            for (let y = 0; y < this.catalogo.currencies.length; y++) {
                                if (this.dataQuotas[w].currency == this.catalogo.currencies[y].id) {
        ...
        //
        obtenerBancos(idCurrency,index) {
            let arreglo = []
            for (let x = 0; x < this.Bancos.length; x++) {
                for (let y = 0; y < this.dataQuotas.length; y++) {
                    for (let z = 0; z < this.dataAccounts.length; z++) {
                        if (this.Bancos[x].is_fintech) {
                            if (this.dataQuotas[y].currency == idCurrency && this.dataAccounts[z].currency == idCurrency && this.dataAccounts[z].status == true) {
                                arreglo.push(this.Bancos[x])
                            }
                        } else {
                            if (this.Bancos[x].id == this.dataQuotas[y].entity.id && this.dataQuotas[y].currency == idCurrency && this.dataAccounts[z].status == true && this.dataAccounts[z].currency == idCurrency && this.dataAccounts[z].entity.id == this.Bancos[x].id) {
                                arreglo.push(this.Bancos[x])
                            }
                        }
                    } 
            ...
            if (this.dataQuotas != null) {
                for (let w = 0; w < this.dataQuotas.length; w++) {
                    for (let x = 0; x < this.Bancos.length; x++) {
                        for (let z = 0; z < this.dataAccounts.length; z++) {
                            if (this.Bancos[x].is_fintech) {
                                if (arreglo.length == 0 && this.dataQuotas[w].currency == idCurrency) {
                                    this.entityArray.bank.person.name = i18n.t("entityArrayNullFintech")
                                    this.entityArray.bank.id = "null"
                                    return this.entityArray
                                }
                            } else {
                                if (arreglo.length == 0 && this.dataQuotas[w].currency != idCurrency && this.dataAccounts[z].currency == idCurrency && this.dataAccounts[z].status) {
                                    this.entityArray.bank.person.name = i18n.t("entityArrayNull")
                                    this.entityArray.bank.id = "null"
                                    return this.entityArray
                                } else if (arreglo.length == 0 && this.dataAccounts[z].currency != idCurrency && this.dataQuotas[w].currency == idCurrency) {
                                    this.entityArray.bank.person.name = i18n.t("entityArrayNullAccountDebtor") 
                                    this.entityArray.bank.id = "null"
                                    return this.entityArray
                                }
                            }
        ...
        //
        obtenerQuotaAvailable: function (id, currency) {
            let available = 0

            if (id == null || id == undefined) return available

            if (this.dataQuotas != null) {
                for (let x = 0; x < this.Bancos.length; x++) {
                    for (let w = 0; w < this.dataQuotas.length; w++) {
                        if (!this.Bancos[x].is_fintech) {
                            if (this.dataQuotas[w].entity.id == id && currency == this.dataQuotas[w].currency) {
                                available = this.dataQuotas[w].available
                            }
                        } else {
                            if (currency == this.dataQuotas[w].currency) {
                                available = this.dataQuotas[w].available
                            }
                        }
                    }
        ...
        //
        obtenerQuotaUsage: function (id, currency) {
            let usage = 0

            if (id == null || id == undefined) return usage
        ...
        //
        obtenerQuotaEntity: function (id, currency) {
            let bankId = ''

            if (id == null || id == undefined) return bankId


            if (this.dataQuotas != null) {
                for (let w = 0; w < this.dataQuotas.length; w++) {
                    if (this.dataQuotas[w].entity.id == id && currency == this.dataQuotas[w].currency) {
                        bankId = this.dataQuotas[w].entity.id
                    }
                }
            }
        ...
        //
        clasePorcentaje: function (index) {
            let porcentaje = 0
            let indicador = ""

            for (let x = 0; x < this.dataQuotas.length; x++) {
                if (this.dataQuotas[x].entity.id == this.idBanco[index]) {
                    porcentaje = (this.dataQuotas[x].usage / this.dataQuotas[x].available * 100.00)
                }
            }

            if (porcentaje >= 0 && porcentaje < 30) {
                indicador = "text-success"
            } else if (porcentaje >= 30 && porcentaje < 60) {
                indicador = "text-primary"
            } else if (porcentaje >= 60 && porcentaje < 80) {
                indicador = "text-warning"
            } else if (porcentaje >= 80 && porcentaje <= 100) {
                indicador = "text-danger"
            }
        ...
        calcularSeleccionadas: function (currency, index) {
            ...
            if (this.dataQuotas != null) {
                for (let x = 0; x < this.Bancos.length; x++) {
                    for (let w = 0; w < this.dataQuotas.length; w++) {
                        if (this.Bancos[x].is_fintech === true) {
                            if (currency == this.dataQuotas[w].currency) {
                                quotaUsage = this.dataQuotas[w].usage
                                quotaAvailable = this.dataQuotas[w].available
                            }
                        } else {
                            if (this.dataQuotas[w].entity.id == this.idBanco[index] && currency == this.dataQuotas[w].currency) {
                                quotaUsage = this.dataQuotas[w].usage
                                quotaAvailable = this.dataQuotas[w].available
                            }
                        }
                    ...
                    for (var t = 0; t < respond.length; t++) {
                        if (respond[t].errors != null) {
                            if (respond[t].errors == "Error al financiar") {
                                ...
                                continue
                            } else if (respond[t].errors == "quotas not available" || respond[t].errors == "quota not found") {
                                if (estado.error) {
                                    toastr.warning(i18n.tc("mensajesModal.estimadoUsuario") + "<br><br>" + i18n.tc("mensajesModal.errorRiskLimitNotAvailable"))
                                    estado.error = false
                                }
                                ...
                                if (estado.error) {
                                    toastr.warning(i18n.tc("mensajesModal.estimadoUsuario") + "<br><br>" + i18n.tc("mensajesModal.errorSupplierNotAcceptedInvitation"))
                                    estado.error = false
                                }
                                continue
