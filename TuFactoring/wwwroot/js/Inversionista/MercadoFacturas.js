new Vue({
    ...
    data: {
        modalLogout: { mostrar: false, vueRefresh : true},
        options: {},
        buscarFacturas: true,
        totalFacturas: 0,
        tamanoTlf: tamanoTlf,
    ...
    methods: {
            ...
            await axios.post('?handler=auction', {},
                ...
                .then((respond) => {
                    ...
                    if (respond.data == null || respond.data.state != "opened") {
                        ...
                        return
                    } else if (respond.data == "Auction not found") {
                        this.facturas = []
                        this.facturasFiltradas = []
                        this.marketNotOpened = true
                        return
                    ...
                    var ruta = "/wsSubastas"
                    startSignalrConnection(ruta , this.loginGroup)
        ...
        async ofertarActual(item) {
            ...
            if (item.discount == '' || item.discount > resp) {
                ...
                if (this.connection.state != 1) {
                    toastr.error(i18n.t("mensajesModal.estimadoUsuario") + ".<br><br>" + i18n.t("notConexionDetected") + "<br>" + i18n.t("verifiqueNuevoIntento"))
                    return
                }
        ...
        entrada: function(message) {
            ...
            if (message != null && message.errors == notAuthorized) {
                window.location.href = "../Logout?returnUrl=~/index?error=sessionExpired"
                return
            } else if (message.errors == "factor don't have accounts") {
                toastr.warning(i18n.t("mensajesModal.estimadoUsuario") + "<br><br>" + i18n.t("mensajesModal.factorDontHaveAccount"))
                return
            } else if (message.errors == "Factor is banned") {
                toastr.warning(i18n.t("mensajesModal.estimadoUsuario") + "<br><br>" + i18n.t("mensajesModal.factorBannedFromMarket"))
                return
            }
        ...
        //
        refrescarSession: async function () {
            await refrescarSession()
            this.token = await refreshTokenAuction(this.token)
        },
        //
        relog: function () {
