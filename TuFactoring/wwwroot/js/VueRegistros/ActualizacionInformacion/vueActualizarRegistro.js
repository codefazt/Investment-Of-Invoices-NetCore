var app = new Vue({
    ...
    methods: {
        limpiarPorRol() {
            ...
            if (this.registro.authRol == false) {
                //Seccion de Errores
        ...
        async ContenidoParaMandar(registro, ruta) {
            ...
            var data = await axios.post(ruta, registroGuardar, { headers: { "RequestVerificationToken": $('input:hidden[name="__RequestVerificationToken"]').val() } })
                .then((respond) => {
                    ...
                    else if (respond.data.error == "You are not authorised to perform this action") {
                    ...
                    else if (respond.data.error == "invalid account number") {
                        toastr.error(i18n.t("mensajesModal.estimadoUsuario") + "<br /><br />" + i18n.t("tooltip.formatoCuentaBancaria"));
                    }
                    else if (respond.data.error == 'account number already exists') toastr.error(i18n.t("mensajesModal.estimadoUsuario") + "<br /><br />" + i18n.t("tooltip.cuentaBancariaExistente"));
                    else if (respond.data.error == "cannot be null") toastr.error(i18n.t("mensajesModal.estimadoUsuario") + "<br/><br/> Usted no cuenta con los datos requeridos del Representante Legal. <br/>Por favor comuníquese con el Operativo de TuF@ctoring, para actualizar su información.");
