new Vue({
    ...
    data: {
        ...
        headers2: [
            ...
            {
                text: i18n.t("headers.documentClient"), value: "debtor.number", align: "center"
            },
            {
                text: i18n.t("headers.numeroFactura"), value: "number", align: "center"
            },
    ...
    methods: {
        async llenarCatalogo() {
            await axios.post('?handler=catalogo', {},
                ...
                .then((respond) => {
                    ...
                    this.tipoDeduccion = this.catalogo.charges;
        ...
        // Funcion para Filtrar las monedas que posee el Proveedor para la carga
        validateAccounts() {
            let availableCurrencies = this.tipoMoneda
            let availableAccounts = this.dataAccounts
            let responseArray = []

            availableAccounts.map(accounts => {
                availableCurrencies.map(data => {
                    if (accounts.currency == data.id && accounts.status == true) {
                        responseArray.push(data)
                    }
                })
            })

            let currencyArray = new Set(responseArray)

            return currencyArray
        },
        //
        async llenarClientes() {

            await axios.post('CargaFacturas?handler=Clientes', {},
                {
                    headers: {
                        "RequestVerificationToken": $('input:hidden[name="__RequestVerificationToken"]').val()
                    }
                ...
                .then((respond) => {
                    ...
                    this.clientes = respond.data

                    this.clientes.sort((a, b) => a.name.toLowerCase() < b.name.toLowerCase() ? -1 : +(a.name.toLowerCase() > b.name.toLowerCase()))
                }).catch((respond) => { console.log(respond); });
        },
        //
        ...
        //
        validarCliente() {
            if (this.nuevo.cliente.id == 0) {
                this.errorCliente = true
                this.errorClasses.cliente = this.isInvalid
            } else {
                this.errorCliente= false
                this.errorClasses.cliente = this.isValid
            }
        },
        //
        validarCurrency() {
        ...
        guardarFactura: async function (factura) {
            ...
            this.dialog = false
            this.envio = true
            await axios.post('?handler=Crear', factura,
                {
                    headers: {
                        "RequestVerificationToken": $('input:hidden[name="__RequestVerificationToken"]').val()
                    }
                ...
                .then((respuesta) => {
                    ...
                    if (resp.errors != null) {
                        ...
                        if (resp.errors == "Invalid User") {
                            ...
                            toastr.warning(i18n.tc("mensajesModal.estimadoUsuario") + "<br><br>" + i18n.tc("facturaExistenteSupplierManual", 2, { 0: resp.number }))
                            errores = i18n.tc("facturaExistente", 2, { 0: resp.number })
                            this.dialog = true
                            this.limpiarFieldErrors();
                        } else if (resp.errors == "term days not valid") {
                            toastr.warning(i18n.tc("mensajesModal.estimadoUsuario") + "<br><br>" + i18n.tc("mensajesModal.fechaVencimientoTermDaysSupplier"))
                            errores = i18n.tc("Error en la Fecha de Vencimiento")
                            this.dialog = true
                            this.limpiarFieldErrors();
                        } else if (resp.errors == "supplier has not accepted invitation") {
                            toastr.warning(i18n.tc("mensajesModal.estimadoUsuario") + "<br><br>" + i18n.tc("mensajesModal.supplierWithoutAcceptInvitation"))
                            errores = i18n.tc("mensajesModal.errorSupplierInvitationWithDebtor")
                            this.dialog = true
                            this.limpiarFieldErrors();
                            ...
                            toastr.warning(i18n.tc("mensajesModal.estimadoUsuario") + "<br><br>" + i18n.tc("mensajesModal.debtorDontHaveAccountCreateInvoice"))
                        ...
                        toastr.success(i18n.t("mensajesModal.estimadoUsuario") + "<br><br>" + i18n.t("mensajesModal.cargaFacturaSupplier"))
                        this.limpiarFieldErrors();
                    }
            ...
            axios.post('?handler=guardar', this.facturas,
                ...
                .then((respond) => {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        if (data[i].errors != null || data[i].errors != undefined) {
                            if (data[i].errors == "invoice found") {
                                errores.push(i18n.tc("facturaExistenteSupplier", 2, { 0: data[i].number }))
                                continue
                            } else if (data[i].errors == "supplier has not accepted invitation") {
                                errores.push(i18n.tc("mensajesModal.errorSupplierInvitationWithDebtor"))
                                continue
                                ...
                                errores.push(i18n.tc("mensajesModal.debtorDontHaveAccountCreateInvoices"))
                                continue
                            }
                    ...
                    if (status == data.length) {
                        toastr.success(i18n.t("mensajesModal.estimadoUsuario") + "<br><br>" + i18n.t("mensajesModal.cargaFacturaMasivaSupplier"))
                    }
        ...
        quitarFacturas: function (facturas, errores) {
            ...
            for (var i = 0; i < this.facturas.length; i++) {
                ...
                this.facturasErroneas.push(
                    {
                        debtor: { number: data.debtor.number },
                        number: data.number,
            ...
            this.facturasErroneas.sort((a, b) => a.debtor.number.toLowerCase() < b.debtor.number.toLowerCase() ? -1 : (a.debtor.number.toLowerCase() > b.debtor.number.toLowerCase()))
        },
        //************************************************************
        ...
        cargarData: function (e) {
            ...
            contenido.map(data => {
                ...
                var facturaActual = {
                    ...
                    amount: factura.length > 5 ? factura[5] : "",
                    message: ""
                }
                ...
                var encontrado = { currency: false, debtor_id: false, number: false }
                ...
                if (contador > 0 && contador == this.catalogo.identifications.length) {
                    facturaActual.message = i18n.t("clienteInvalido2")
                    this.facturasErroneas.push(facturaActual)
                    return
                }

                for (let i = 0; i < this.clientes.length; i++) {
                    if (this.clientes[i].documents[0].number == factura[0]) {
                        factura[0] = { debtor_id: this.clientes[i].id, name: this.clientes[i].name, number: factura[0] }
                        encontrado.debtor_id = true
                        break
                    }
                }
                ...
                if (!encontrado.currency || !encontrado.debtor_id) {
                    let msg = ""
                    if (!encontrado.currency)
                        msg = "tipoMoneda"

                    if (!encontrado.debtor_id)
                        msg = "clienteInvalido"
                ...
                try {
                    var facturaVerificar = {
                        debtor_id: factura[0].debtor_id,
                        debtor: { name: factura[0].name, number: factura[0].number },
                        number: factura[1],
        ...
        validarFactura: function (data) {
            ...
            /*this.dataAccounts.map(accounts => {
                if (accounts.currency == data.currency_id && accounts.status == true) {
                    validator = validator + 1
                }
            })

            if (validator == 0) {
                this.msg = "mensajesModal.supplierDontHaveAccountCreateInvoices"
                return true
            }*/
            ...
            /*if (moment(data.expiration_date, "YYYY-MM-DD").diff(ahora.format("YYYY-MM-DD"), "days") < 30) {
                this.msg = "mensajesModal.fechaVencimientoMenor5"
                return true;
            }*/

            if (moment(data.expiration_date, "YYYY-MM-DD").diff(moment(data.issued_date , "YYYY-MM-DD"), "days") < 30) {
                this.msg = "mensajesModal.fechaVencimientoMenor5"
                return true;
            }
        ...
        agregarFactura: function () {
            ...
            if (!this.encuentraErrorFactura()) {
                ...
                this.enviarFactura({
                    ...
                    debtor_id: this.nuevo.cliente.id,
        ...
        encuentraErrorFactura: function () {
            ...
            if (this.nuevo.cliente.id == '0' || this.nuevo.cliente.id == '') {
                this.message = "clienteInvalido2"
                return true;
            }
    ...
    created: function () {
        document.getElementById("appCargaFacturas").removeAttribute("hidden")
