new Vue({
    el: "#appConsultasPagosFacturasCompradas",
    i18n,
    vuetify: new Vuetify({
        lang: {
    ...
    data: {
        ...
        headersReceipts: [
            ...
            //{ text: i18n.t("headers.originalAmount"), value: "originalamount", align: "center" },
            ...
            { text: i18n.t("headers.details"), value: "detail", align: "center" },
        ],
        headerFacturasFactor: [
            ...
            { text: i18n.t("headers.amountPaid"), value: "payable", align: "center" },
            { text: i18n.t("headers.comisionServicio"), value: "commission", align: "center" },
            
        ],
        headerFacturasVencidas: [
            { text: i18n.t("headers.n"), value: "n", align: "center" },
        ...
        headerNoPayments: [
            ...
            { text: i18n.t("headers.amountPayment"), value: "amount", align: "center" },
            { text: i18n.t("headers.estatus"), value: "state", align: "center" },
            { text: i18n.t("headers.reason"), value: "message", align: "center" },
        ],
        accounts: [],
        receipts: [],
        banks: [],
    ...
    methods: {
        ...
        async payedInvoices(factura) {
            ...
            this.loading = true
            let take = 100;
            let provCurrency = null

            const data = await axios.post('?handler=PayedInvoices', { pagination: { take: take, skip: this.totalFacturas }, filter: this.filter }, { headers: { "RequestVerificationToken": $('input:hidden[name="__RequestVerificationToken"]').val() } })
        ...
        async unpayedInvoices(factura) {
            ...
            this.loading = true
            let take = 100;
            const data = await axios.post('?handler=UnpayedInvoices', { pagination: { take: take, skip: this.totalFacturas }, filter: this.filter }, { headers: { "RequestVerificationToken": $('input:hidden[name="__RequestVerificationToken"]').val() } })
                .then(function (response) {
                    // handle success
                    resetTime()
                    return response;
        ...
        DetallePagos(item) {
            ...
            if (item.state == "paid") this.dialogDetailsPayments = true
            else this.dialogDetailsNoPayments = true
    ...
    watch: {
        options: {
            async handler() {
                ...
                console.log(this.buscarFacturas)
                if (this.buscarFacturas && this.options.page * this.options.itemsPerPage >= this.respondQuery.length - this.options.itemsPerPage) {
                    await this.payedInvoices();
                }
            },
            deep: true,
