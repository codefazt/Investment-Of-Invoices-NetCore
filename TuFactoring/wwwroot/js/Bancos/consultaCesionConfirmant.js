var app = new Vue({
    ...
    data: {
        ...
        headerInvoices: [
            ...
            { text: i18n.t("headers.proveedor"), value: 'supplier', align: 'center', sortable: false },
            ...
            { text: i18n.t("headers.operationCesion"), value: 'changelogs', align: 'center', sortable: false },
            { text: i18n.t("headers.monto"), value: 'amount', align: 'center', sortable: false },
            { text: i18n.t("headers.doct"), value: 'options', align: 'center', sortable: false },
        ],
        moment: moment,
        loading: false,
        options: {},
    ...
    methods: {
        ...
        async getFile(item) {
            const data = await axios.post("?handler=GetPDF", { id: item.id }, { headers: { "RequestVerificationToken": $('input:hidden[name="__RequestVerificationToken"]').val() } })
                .then(function (response) {
                    resetTime();
                    if (item.id != null) {
                        ...
                        console.log(fullUrl)
                        window.open(
                            fullUrl + "/files/RAC_" + item.id + ".pdf", "_blank"
                        )
                    }
                    return
                });
