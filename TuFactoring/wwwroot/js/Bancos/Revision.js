new Vue({
    ...
    methods: {
        ...
        confirmingInvoice: async function (indice) {
            this.cargando = true;
            await this.confirmarFactura(this.facturas[indice], "Confirma");

            this.cargando = false;
        },
        // Nuevas Mutaciones reviewInvoice y refuseInvoice
        reviewInvoice: async function (indice) {
            this.cargando = true;
            await this.confirmarFactura(this.facturas[indice], "ReviewInvoice", "tooltip.reviewFacturaIndividual");

            this.cargando = false;
        },
        refuseInvoice: async function (indice) {
            this.cargando = true;
            await this.confirmarFactura(this.facturas[indice], "RefuseInvoice", "tooltip.refuseFacturaIndividual");

            this.cargando = false;
        },
        //-----------------------------------------
        confirmarFactura: async function (factura, ruta, respuesta) {
            ...
            let status = 0
            this.envio = true
            await axios.post('?handler=' + ruta, listado,
                {
                    headers: {
                        "RequestVerificationToken": $('input:hidden[name="__RequestVerificationToken"]').val()
                    }
                ...
                .then((respond) => {
                    ...
                    respond.map(data => {
                        if (data.errors != null) {
                            ...
                            if (data.errors == "field [terms_days] not valid") {
                                toastr.warning(i18n.tc("mensajesModal.estimadoUsuario") + "<br><br>" + i18n.tc("tooltip.termDays30"))
                                return
                            }
                    ...
                    if (status == respond.length) toastr.success(i18n.t("mensajesModal.estimadoUsuario") + "<br><br>" + i18n.t(respuesta))
        ...
        //
        confirmarFacturas: async function (ruta, respuesta) {
            ...
            await axios.post('?handler=' + ruta, listado,
                ...
                .then((respond) => {
                    ...
                    if (status == respond.data.length) toastr.success(i18n.t("mensajesModal.estimadoUsuario") + "<br><br>" + i18n.t(respuesta))
                    else if (status > 0) toastr.warning(i18n.t("problemasRespuesta"))
                    else toastr.warning(i18n.t("errorRespuesta"))
