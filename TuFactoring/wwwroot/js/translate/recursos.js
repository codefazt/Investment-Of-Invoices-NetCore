...
//
function obtenerLangAmounts(lang) {
    switch (lang) {
        case "ESD": return "en"; break
        case "ESS":
        case "ESV":
        case "es": return "es"; break
        case "ENU":
...
async function refrescarSession() {
    ...
    await axios.post('../Profile/recursos?handler=ping', {},
        {
            headers: {
                "RequestVerificationToken": $('input:hidden[name="__RequestVerificationToken"]').val()
            }
...
async function refreshTokenAuction(tokenActual) {
    var token = tokenActual

    await axios.post('../Profile/recursos?handler=token', {},
        {
            headers: {
                "RequestVerificationToken": $('input:hidden[name="__RequestVerificationToken"]').val()
            }
...
function isMyOffert(publication, owner) {
    try {
        if (publication.bids[0].factor.id == owner) {
            return true
        }
    } catch (e) {
        return false
...
function recallVueArray(array) {
    array.push(true)
    array.pop()
}

function getDateStatus(array, status) {
    var date = moment()

    for (var i = 0; i < array.length; i++) {
        if (array[i].to == status) {
            date = array[i].changedAt
            break;
        }
    }
...
function isStatus(array,status) {
...
function filterIsEmpty(filter, ignores) {
    ...
    ignores.map(ignore => {
        arr = arr.filter(data =>  data != ignore )
    })

    return arr.length == 0
}

function filtersIsEmpty(filters, ignores) {
    var resp = true

    if (filters == null || filters.length == 0) return resp

    for (var i = 0; i < filters.length; i++) {
        resp = filterIsEmpty(filters[i], ignores)
...
function iniciarButtonFilters() {

    try {
        for (var i = 0; i < 5; i++) {
            var b = document.getElementById("buttonFilterClear-" + i)
            
            if(b == null) continue
            
            b.addEventListener("click", function (e) {
                Array.from(document.getElementsByClassName("clear-" + e.target.value)).map(data => {
                    data.value = ""
                })
            })
        }
...
const CutEmail = x => {
    ...
    try {
        ...
        console.log("A ocurrido un error al dividir el Correo")
    }
}
